...
// determine if 2 circles overlap, and provide contact details
export default function circleCircleOverlap(
    centerA,
    radiusA,
    centerB,
    radiusB,
    ...
    if (d !== 0.0) {
        ...
        // Special case. circles are exactly on top of each other. Prevent divide by zero.
        d = r - 1.0;
        vec2.set(r, 0.0, _delta);
        vec2.scale(_delta, (r - d) / d, _mtd);
    }

    // delta is the overlap between the two circles, and is a vector that can be added to
    // circle A’s position to move them into a non-colliding state.
    vec2.copy(_mtd, contact.delta);

    // position is the point of contact of these 2 circles (assuming they no longer penetrate)
