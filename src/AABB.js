...
export function fromPoints(points) {
    if (!points.length) throw new Error("no points specified");
...
export function fromSegments(segments, indices) {
    if (!segments.length) throw new Error("no segments specified");

    if (!indices.length) throw new Error("no indices specified");
...
// create an AABB containing a circle
export function fromCircle(center, radius) {
    return {
