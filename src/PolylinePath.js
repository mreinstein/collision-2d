...
// of a circle at each point and a cylinder along each segment.
...
export function mapPointToPathDistance(path, point) {
    ...
    for (let i = 1; i < path.pointCount; i++) {
        ...
        if (minDistance === undefined || d < minDistance) {
            minDistance = d;
            pathDistance =
                segmentLengthTotal + tmpSegDistance.segmentProjection;
        }
...
export function mapPathDistanceToPoint(out, path, pathDistance) {
    ...
    if (path.cyclic) {
        ...
        if (pathDistance < 0) return vec2.copy(path.points[0], out);
